...
public class User {
    ...
    private Integer employeeId; // 员工ID（主键）
    ...
    private byte[] profilePicture; // 头像，使用 BLOB 存储（存储为二进制数据）
  ...
  @Enumerated(EnumType.STRING)
    ...
    private jobType jobType; // 职位类型（检查人员，招标人员，项目经理，后台管理员）

    @Column(name = "hire_date", updatable = false)
    private LocalDate hireDate; // 入职时间（使用 LocalDate）
