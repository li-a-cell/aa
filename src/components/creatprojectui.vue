...
export default {
  ...
  setup() {
    ...
    const form = ref({
      ...
      plannedEndDate: '',
      budget: '',
      projectDescription: '',  // 这里初始值先留空，后续会处理文件相关
      siteName: '',
      status: '',
      projectType: '',
    });
    ...
    // 新增函数，用于处理文件选择后的操作，获取文件对象
    const handleFileChange = (e) => {
      const file = e.target.files[0];
      form.value.projectDescription = file;  // 将文件对象赋值给对应的属性，后续传给后端
    };

    const submitProject = async () => {
      try {
        form.value.budget = String(form.value.budget);
        const formData = new FormData();  // 创建FormData对象来处理包含文件的表单数据
        for (const key in form.value) {
          formData.append(key, form.value[key]);
        }
        const response = await axios.post('/api/manager/createProject', formData, {
          headers: {
            'Content-Type': 'multipart/form-data',  // 修改Content-Type为适合文件上传的类型
            'token': token
          },
        });
        console.log('创建项目成功！');
